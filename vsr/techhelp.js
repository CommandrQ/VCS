...
async function loadTiers() {
    ...
    tiers.forEach(tier => {
        ...
        if (tier.studentPromo && tier.studentPromo.active) {
            const deadline = new Date(tier.studentPromo.deadline);
            if (now <= deadline) {
...
function submitRequest() {
    ...
    if (!name || !phone || !issue) { alert("Please fill out all fields."); return; }
    ...
    if (isUserSignedIn) {
        ...
        if (adult !== "YES") { alert("Adult verification required."); return; }
        body += `\n\n[LOGISTICS]\nAddress: ${address}\nAdult Present: YES`;
    }
